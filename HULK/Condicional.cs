...
public class If_else: Utiles1
{
    public static string ReturnIf_else(string instruction)
    {
        ...
        else return "\"Condición booleana inválida\"";
    }
    ...
    private static (string, int, int) Parse_condition(string instruction, int index)
    {
        ...
        if (final == 0) return ("\"Se esperaba )\"", 0, 0);
        ...
        if (contador > 0) return ("\"Se requieren ambas estructuras: if y else. Faltó una cantidad: " + contador + " de estructuras else\"", 0, 0);
        if (contador < 0) return ("\"Se requieren ambas estructuras: if y else. Faltó una cantidad: " + Math.Abs(contador) + " de estructuras if\"", 0, 0);
