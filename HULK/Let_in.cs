...
public class Let_in: Utiles2
{
    public static string Return_Let_in(string instruction)
    {
        ...
        for (int i = 0; i < variables.Length; i++)
        {
            ...
            if(!Es_un_nombre_valido(dos_partes[0])) return "\"Se esperaba un identificador, " + dos_partes[0] + " identificador inválido\"";
            if(Es_palabra_reservada(dos_partes[0])) return "\"No se pueden utilizar palabras reservadas como identificadores, " + dos_partes[0] + "\"";
    ...
    private static (string[], string) Parse(string instruction)
    {
        ...
        for (int i = 0; i < instruction.Length; i++)
        {
            ...
            else if (i < instruction.Length - 4 && instruction[i] == ' ' && instruction[i + 1] == 'i' && instruction[i + 2] == 'n' && instruction[i + 3] == ' ')
            {
                ...
                return (new String[0], "\"Se esperaba una estructura let \"");
            }
        }

        if(balancear >0)
                return (new String[0], "\"Se esperaba una estructura in\"");
